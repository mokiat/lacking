	...
	When("items are inserted", func() {
		...
		BeforeEach(func() {
			firstItemID = tree.Insert(
				shape2d.NewCompactQuadFromCircle(
					shape2d.NewCircle(dprec.NewVec2(16.0, 16.0), 2.0),
				),
				"First",
			)
			secondItemID = tree.Insert(
				shape2d.NewCompactQuadFromCircle(
					shape2d.NewCircle(dprec.NewVec2(48.0, 48.0), 2.0),
				),
				"Second",
			)
			thirdItemID = tree.Insert(
				shape2d.NewCompactQuadFromCircle(
					shape2d.NewCircle(dprec.NewVec2(-16.0, -48.0), 32.0),
				),
				"Third",
			)
		})
