...
// being edited.
func AddComponent[T any](op *EditOperation, value T) {
	tIndex := getTypeIndex[T](op.scene)
	if op.mask.containsType(tIndex) {
		panic("entity already has component of this type")
	}
	op.mask.addType(tIndex)

	storage := getStorageFromIndex[T](op.scene, tIndex)
	storage.pendingValue = value
}

// RemoveComponent removes the component of type T from the entity being edited.
func RemoveComponent[T any](op *EditOperation) {
	tIndex := getTypeIndex[T](op.scene)
	if !op.mask.containsType(tIndex) {
		panic("entity does not have component of this type")
	}
	op.mask.removeType(tIndex)
}
