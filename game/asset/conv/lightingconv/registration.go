...
func init() {
	dsl.RegisterConverter(lightingdto.LightingChunkID, &Converter{})
}
...
func (c *Converter) Convert(asset dsl.Resource) (chunked.Chunk, error) {
	src := asset.(Source)
	chunk, err := CreateLightingChunk(src)
	if err != nil {
		return nil, err
	}
	return chunked.FromValue(lightingdto.LightingChunkID, chunk), nil
}
