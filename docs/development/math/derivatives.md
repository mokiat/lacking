...
| Sum | $(x+y)'$ | $x' + y'$ |
| Product | $(xy)'$ | $xy'+x'y$ |
| Quotient | $(\frac{x}{y})'$ | $\frac{x'.y-x.y'}{y^2}$ |
...
| Multivariable rule | $\frac{\partial f(u(x), v(x))}{\partial x}$ | $\frac{\partial f(u(x), v(x))}{\partial u(x)}\frac{\partial u(x)}{\partial x} + \frac{\partial f(u(x), v(x))}{\partial v(x)}\frac{\partial v(x)}{\partial x}$ |
