package mat

import "github.com/mokiat/lacking/ui"

var (
	PrimaryColor        ui.Color = ui.RGB(0x21, 0x96, 0xF3)
	OnPrimaryColor      ui.Color = ui.RGB(0x00, 0x00, 0x00)
	PrimaryLightColor   ui.Color = ui.RGB(0x00, 0x00, 0x00) // TODO
	OnPrimaryLightColor ui.Color = ui.RGB(0x00, 0x00, 0x00) // TODO
	PrimaryDarkColor    ui.Color = ui.RGB(0x19, 0x76, 0xD2)
	OnPrimaryDarkColor  ui.Color = ui.RGB(0x19, 0x76, 0xD2) // TODO

	SecondaryColor        ui.Color = ui.RGB(0x8B, 0xC3, 0x4A)
	OnSecondaryColor      ui.Color = ui.RGB(0x00, 0x00, 0x00)
	SecondaryLightColor   ui.Color = ui.RGB(0x00, 0x00, 0x00) // TODO
	OnSecondaryLightColor ui.Color = ui.RGB(0x00, 0x00, 0x00) // TODO
	SecondaryDarkColor    ui.Color = ui.RGB(0x68, 0x9F, 0x38)
	OnSecondaryDarkColor  ui.Color = ui.RGB(0x68, 0x9F, 0x38) // TODO

	BackgroundColor   ui.Color = ui.RGB(0xFF, 0xFF, 0xFF)
	OnBackgroundColor ui.Color = ui.RGB(0xFF, 0xFF, 0xFF) // TODO

	SurfaceColor        ui.Color = ui.RGB(0xFF, 0xFF, 0xFF)
	OnSurfaceColor      ui.Color = ui.RGB(0x00, 0x00, 0x00)
	OnSurfaceHoverColor ui.Color = ui.RGB(0xDD, 0xDD, 0xDD)
	OnSurfacePressColor ui.Color = ui.RGB(0x66, 0x66, 0x66)

	ErrorColor   ui.Color = ui.RGB(0xB0, 0x00, 0x20)
	OnErrorColor ui.Color = ui.RGB(0xFF, 0xFF, 0xFF)

	OutlineColor ui.Color = ui.RGB(0x80, 0x80, 0x80)
)
