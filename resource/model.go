...
func (o *ModelOperator) Allocator(uri string) Allocator {
	return AllocatorFunc(func(set *Set) (interface{}, error) {
		in, err := o.locator.Open(uri)
		if err != nil {
			return nil, fmt.Errorf("failed to open model asset %q: %w", uri, err)
		}
		defer in.Close()

		modelAsset := new(asset.Model)
		if err := asset.DecodeModel(in, modelAsset); err != nil {
			return nil, fmt.Errorf("failed to decode model asset %q: %w", uri, err)
		}
		...
		for i, meshAsset := range modelAsset.Meshes {
			mesh, err := AllocateMesh(set, o.gfxWorker, &meshAsset)
			if err != nil {
...
func (o *ModelOperator) Releaser() Releaser {
	return ReleaserFunc(func(resource interface{}) error {
		model := resource.(*Model)

		for _, mesh := range model.meshes {
			if err := ReleaseMesh(o.gfxWorker, mesh); err != nil {
