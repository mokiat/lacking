...
//
// Unlike [Octree], a Bag performs no spatial subdivision, so the cost of a
...
// as [Octree], so the two can be swapped with minimal changes to calling code.
...
// function panics.
func (b *Bag[T]) Insert(aabb shape3d.AABB, value T) BagItemID {
	if aabb.IsEmpty() {
...
func (b *Bag[T]) Update(id BagItemID, aabb shape3d.AABB) {
	if aabb.IsEmpty() {
...
func (b *Bag[T]) QuerySegment(segment shape3d.Segment, yield VisitorFunc[T]) {
	for i := range b.items {
		item := &b.items[i]
...
// between invocations.
func (b *Bag[T]) QueryAABB(aabb shape3d.AABB, yield VisitorFunc[T]) {
	for i := range b.items {
		item := &b.items[i]
...
func (b *Bag[T]) QueryFrustum(frustum shape3d.Frustum, yield VisitorFunc[T]) {
	for i := range b.items {
		item := &b.items[i]
