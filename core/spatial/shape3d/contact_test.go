...
var _ = Describe("Contact", func() {
	var contact shape3d.Contact

	BeforeEach(func() {
		contact = shape3d.Contact{
			TargetPoint:  dprec.NewVec3(10.0, 0.0, 0.0),
			TargetNormal: dprec.NewVec3(1.0, 0.0, 0.0),
			Depth:        2.0,
		}
	})
	...
	Describe("EvalSourcePoint", func() {
		It("lies Depth away from TargetPoint along the inverse normal", func() {
			Expect(contact.EvalSourcePoint()).To(dprectest.HaveVec3Coords(8.0, 0.0, 0.0))
		})
	})

	Describe("EvalSourceNormal", func() {
		It("is the inverse of TargetNormal", func() {
			Expect(contact.EvalSourceNormal()).To(dprectest.HaveVec3Coords(-1.0, 0.0, 0.0))
		})
	})
	...
	Describe("Flipped", func() {
		It("promotes the source point and normal to the target", func() {
			...
			Expect(flipped.TargetPoint).To(dprectest.HaveVec3Coords(8.0, 0.0, 0.0))
			Expect(flipped.TargetNormal).To(dprectest.HaveVec3Coords(-1.0, 0.0, 0.0))
		...
		It("round-trips back to the original when applied twice", func() {
			...
			Expect(result.TargetPoint).To(dprectest.HaveVec3Coords(contact.TargetPoint.X, contact.TargetPoint.Y, contact.TargetPoint.Z))
			Expect(result.TargetNormal).To(dprectest.HaveVec3Coords(contact.TargetNormal.X, contact.TargetNormal.Y, contact.TargetNormal.Z))
			Expect(result.Depth).To(BeNumerically("~", contact.Depth, 1e-6))
		})

		It("swaps the roles of the source and target points", func() {
			flipped := contact.Flipped()
			Expect(flipped.EvalSourcePoint()).To(dprectest.HaveVec3Coords(contact.TargetPoint.X, contact.TargetPoint.Y, contact.TargetPoint.Z))
		})
	})
})

func newContact(depth float64) shape3d.Contact {
	return shape3d.Contact{
		TargetPoint:  dprec.NewVec3(depth, 0.0, 0.0),
		TargetNormal: dprec.NewVec3(1.0, 0.0, 0.0),
		Depth:        depth,
	}
}
...
var _ = Describe("ContactList", func() {
	var sink shape3d.ContactList

	BeforeEach(func() {
		sink = shape3d.ContactList{}
	})
	...
	It("does not grow its backing array when pre-sized with make", func() {
		list := make(shape3d.ContactList, 0, 4)
		capBefore := cap(list)
		for i := range 4 {
...
var _ = Describe("DeepestContact", func() {
	var sink shape3d.DeepestContact

	BeforeEach(func() {
		sink = shape3d.DeepestContact{}
	})
...
var _ = Describe("ShallowestContact", func() {
	var sink shape3d.ShallowestContact

	BeforeEach(func() {
		sink = shape3d.ShallowestContact{}
	})
