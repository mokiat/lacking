...
func (r *Renderer) renderItem(sequence Sequence, item Item) {
	...
	if item.Program.MetalnessTwoDTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.MetalnessTwoDTexture.ID())
		gl.Uniform1i(item.Program.MetalnessTwoDTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.RoughnessTwoDTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.RoughnessTwoDTexture.ID())
		gl.Uniform1i(item.Program.RoughnessTwoDTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.AlbedoTwoDTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.AlbedoTwoDTexture.ID())
		gl.Uniform1i(item.Program.AlbedoTwoDTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.AlbedoCubeTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.AlbedoCubeTexture.ID())
		gl.Uniform1i(item.Program.AlbedoCubeTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.AmbientReflectionTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.AmbientReflectionTexture.ID())
		gl.Uniform1i(item.Program.AmbientReflectionTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.AmbientRefractionTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.AmbientRefractionTexture.ID())
		gl.Uniform1i(item.Program.AmbientRefractionTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.NormalTwoDTextureLocation != -1 {
		gl.BindTextureUnit(textureUnit, item.NormalTwoDTexture.ID())
		gl.Uniform1i(item.Program.NormalTwoDTextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.FBColor0TextureLocation != -1 {
		gl.ActiveTexture(gl.TEXTURE0 + textureUnit)
		gl.BindTexture(gl.TEXTURE_2D, sequence.SourceFramebuffer.AlbedoTextureID)
		gl.Uniform1i(item.Program.FBColor0TextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.FBColor1TextureLocation != -1 {
		gl.ActiveTexture(gl.TEXTURE0 + textureUnit)
		gl.BindTexture(gl.TEXTURE_2D, sequence.SourceFramebuffer.NormalTextureID)
		gl.Uniform1i(item.Program.FBColor1TextureLocation, int32(textureUnit))
		textureUnit++
	}
	if item.Program.FBDepthTextureLocation != -1 {
		gl.ActiveTexture(gl.TEXTURE0 + textureUnit)
		gl.BindTexture(gl.TEXTURE_2D, sequence.SourceFramebuffer.DepthTextureID)
		gl.Uniform1i(item.Program.FBDepthTextureLocation, int32(textureUnit))
		textureUnit++
	}
