...
func (s *Shader) Allocate(info ShaderAllocateInfo) error {
	s.id = gl.CreateShader(info.ShaderType)
	if s.id == 0 {
		return fmt.Errorf("failed to allocate shader")
	}
	s.setSourceCode(info.SourceCode)
	gl.CompileShader(s.id)
	if !s.isCompileSuccessful() {
		return fmt.Errorf("failed to compile shader: %s", s.getInfoLog())
	}
	return nil
}

func (s *Shader) Release() error {
